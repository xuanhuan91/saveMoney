    ...
    <div style="margin: 30px">
        ...
        <table class="table">
            ...
            <tr>
                ...
                @if(count($parentCategory->subcategory))
                    <td>@include('CategoryExpense.sub_category_list',['subcategories' => $parentCategory->subcategory])</td>
                @endif
                <td>{{$parentCategory->created_at}}</td>
                <td><a class="btn btn-warning">Edit</a></td>
                <td>
                    <form method="post" action="{{route('CategoryExpense.destroy', $parentCategory->id)}}"
